...
export default function VisualEditAgent() {
  ...
  const currentHighlightedElementsRef = useRef<HTMLElement[]>([]); // Multiple elements for hover
  ...
  const findElementsById = (id: string, useSourceLocation?: boolean) => {
    if (!id) return [];
  ...
  const handleElementClick = (e: MouseEvent) => {
    ...
    const elementData = {
      ...
      classes: (element as any).className?.baseVal || element.className || "",
    ...
    elements.forEach((element) => {
      if (replace) {
        ...
        const currentClasses =
          (element as any).className?.baseVal || element.className || "";
  ...
  useEffect(() => {
    ...
    elementsWithLineNumber.forEach((el, index) => {
      ...
      const id = `visual-id-${element.dataset.filename}-${element.dataset.linenumber}-${index}`;
      element.dataset.visualSelectorId = id;
    });
