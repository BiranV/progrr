...
export default function OpeningHoursPage() {
  ...
  const dayLabels = React.useMemo(
    () => dict?.onboarding?.dayLabels || DAY_LABELS,
    [dict],
  );
  ...
  const showFullPageSpinner =
    isPending && !onboardingRes && !initialRef.current;
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (e?.status === 409) {
        window.alert(e?.message || t("openingHours.hoursConflictMessage"));
        return;
      }
  ...
  const addRange = (day: number) => {
    setAvailability((prev) => {
      const nextDays = prev.days.map((d) => {
        ...
        const ranges =
          Array.isArray(d.ranges) && d.ranges.length
            ? d.ranges
            : [{ id: newId(), start: "09:00", end: "17:00" }];
              ...
              <div className="grid grid-cols-[48px_48px_1fr_40px] items-start gap-x-2 p-3 rounded-xl border border-gray-200 dark:border-gray-800 bg-white/50 dark:bg-gray-950/20">
                <div className="h-8 flex items-center justify-center">
                  <Switch
                    ...
                    aria-label={t("openingHours.aria.toggleDay", {
                      day: getDayLabel(d.day),
                    })}
                    disabled={isSaving || (isPending && !initialRef.current)}
                  />
                  ...
                  {(d.ranges || []).map((r, idx) => (
                    ...
                    >
                      <TimePicker
                        ...
                        onChange={(v) =>
                          updateRangeTime(d.day, r.id, "start", v)
                        }
                        disabled={
                          isSaving ||
                        ...
                        aria-label={
                          idx === 0
                            ...
                            : t("openingHours.aria.additionalStartTime", {
                                day: getDayLabel(d.day),
                              })
                        }
                      />
                      ...
                      <TimePicker
                        ...
                        aria-label={
                          idx === 0
                            ...
                            : t("openingHours.aria.additionalEndTime", {
                                day: getDayLabel(d.day),
                              })
                        }
                      />
                    ...
                    >
                      {idx === 0 ? (
                        <Button
                          ...
                          aria-label={t("openingHours.aria.addRange", {
                            day: getDayLabel(d.day),
                          })}
                        >
                        ...
                        <Button
                          ...
                          aria-label={t("openingHours.aria.deleteRange", {
                            day: getDayLabel(d.day),
                          })}
                        >
