...
function OnboardingContent() {
  ...
  const MIN_SERVICE_DURATION = 1;
  const sanitizeDurationDraft = (value: string) => value.replace(/[^0-9]/g, "");
  const normalizeDurationOnCommit = (raw: string, fallback: number) => {
    const trimmed = raw.replace(/^0+(?=\d)/, "");
    if (!trimmed)
      return Math.max(MIN_SERVICE_DURATION, fallback || MIN_SERVICE_DURATION);
    ...
    try {
      ...
      if (step === 1) {
        ...
        if (!data.business?.phone?.trim())
          newErrors.businessPhone = t(
            "onboarding.errors.businessPhoneRequired",
          );
        ...
        if (!String(data.availability?.timezone ?? "").trim())
          newErrors.businessTimezone = t(
            "onboarding.errors.businessTimezoneRequired",
          );
      ...
      if (step === 2) {
        ...
        if (currency === OTHER_CURRENCY_CODE) {
          if (!data.customCurrency?.name?.trim())
            newErrors.currencyName = t(
              "onboarding.errors.currencyNameRequired",
            );
          if (!data.customCurrency?.symbol?.trim())
            newErrors.currencySymbol = t(
              "onboarding.errors.currencySymbolRequired",
            );
        }
        ...
        services.forEach((s) => {
          if (!s.name?.trim())
            newErrors[`serviceName_${s.id}`] = t(
              "onboarding.errors.serviceNameRequired",
            );
          if (!s.durationMinutes || s.durationMinutes <= 0)
            newErrors[`serviceDuration_${s.id}`] = t(
              "onboarding.errors.serviceDurationRequired",
            );
        });
                    ...
                    <Button
                      ...
                      onClick={() =>
                        removeLogo().catch((err) =>
                          setError(
                            err?.message || t("onboarding.removeLogoFailed"),
                          ),
                        )
                      }
                    >
                    ...
                    <Button
                      ...
                      onClick={() =>
                        removeBanner().catch((err) =>
                          setError(
                            err?.message || t("onboarding.removeBannerFailed"),
                          ),
                        )
                      }
                    >
                      ...
                      <div className="w-[70px] shrink-0">
                        <Input
                          ...
                          onBlur={() => {
                            ...
                            const nextValue = normalizeDurationOnCommit(
                              raw,
  ...
  const card = (
    <div className="min-h-screen w-full bg-white flex flex-col pb-safe">
      {/* Header Container */}
      <div className="relative w-full z-0 shrink-0 h-[10vh] bg-gradient-to-br from-[#165CF0] via-[#1E6CF2] to-[#2B79F5] rounded-b-[40px] overflow-hidden">
      ...
      <ImageCropperModal
        ...
        onConfirm={async (cropped) => {
          try {
            ...
            setError(err?.message || t("onboarding.errors.uploadImageFailed"));
            throw err;
          }
        }}
      />
