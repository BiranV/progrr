...
const MAX_BYTES = 5 * 1024 * 1024; // 5MB (banner)
...
export async function POST(req: Request) {
  try {
    ...
    const existingPublicId = String(
      (existingUser as any)?.onboarding?.branding?.banner?.publicId ?? ""
    ).trim();
    ...
    // We generate a wide banner URL sized for the header.
    // The UI uses object-fit: cover / background-size: cover.
...
export async function DELETE() {
  try {
    ...
    const currentPublicId = String(
      (user as any)?.onboarding?.branding?.banner?.publicId ?? ""
    ).trim();

    await c.users.updateOne(
      { _id: new ObjectId(userId) },
      {
        $unset: { "onboarding.branding.banner": "" },
