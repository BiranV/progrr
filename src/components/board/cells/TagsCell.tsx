...
export default function TagsCell({ value, onUpdate, options }: TagsCellProps) {
  ...
  const handleRemoveTag = (tagValueToRemove: string) => {
  ...
  const handleCreateNewTag = () => {
    if (
      newTagInput.trim() &&
      !currentTags.find(
        (t) => t.label.toLowerCase() === newTagInput.trim().toLowerCase()
      )
    ) {
      const newTag: Tag = {
  ...
  const filteredSuggestions = tagSuggestions.filter(
    (suggestion) =>
      !currentTags.find((t) => t.value === suggestion.value) &&
        ...
        <PopoverContent className="w-56 p-2 space-y-2">
          ...
          {newTagInput.trim() &&
            !filteredSuggestions.find(
              (s) => s.label.toLowerCase() === newTagInput.toLowerCase()
            ) && (
              <Button
                variant="outline"
